...
### 4.8 operation_logs（第二版预留）

该表由初始迁移预先建立并继续保留，但第一版业务代码不读写。第二版用于记录关键业务动作，不记录普通页面访问。
...
- 提交与审核：第一版任务状态和通知在同一事务中完成；第二版接入日志时再扩展事务。
- 移除成员：若成员仍有未完成任务，第一版应拒绝移除并返回明确错误。
- 项目归档：采用状态归档，不物理删除项目及其历史数据。
- 用户、任务和通知第一版不提供物理删除接口；预留日志表不提供业务接口；评论和文件删除必须先校验权限及资源归属。
