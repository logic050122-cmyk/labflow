...
> 用户注册登录 -> 创建项目 -> 邀请码加入项目 -> 项目成员管理 -> 任务创建与分配 -> 任务状态流转 -> 任务提交审核 -> 评论 -> 文件上传 -> 站内通知
...
- 涉及数据库表：`projects`、`project_members`、`users`。
- 完成标准：邀请码唯一且可刷新；无效邀请码返回明确错误；用户不能重复加入同一项目；加入后角色固定为 Member；加入通知已在模块 10 接入，操作日志留到第二版。
...
- [x] 第四步：实现 Owner 移除 Member 的后端接口及未完成任务校验
- [x] 第五步：在成员列表接入移除确认、成功刷新和权限按钮显示
- [x] 第六步：完成双用户权限验收、文档同步并标记模块完成
...
个人中心暂不归入第一版或第二版，等待后续单独讨论。

## 7. 第一版最终验收

- [x] 登录后页面统一使用应用布局，并完成桌面端/移动端导航、核心空状态和项目列表分页。
- [ ] 完整跑通“注册登录 -> 创建项目 -> 邀请码加入 -> 成员管理 -> 分配任务 -> 开始处理 -> 提交 -> 审核 -> 评论/附件 -> 通知”。
